  ...
  return (
    <nav className="container mx-auto flex w-full items-center justify-between gap-x-5 bg-black bg-transparent p-2 px-7 lg:px-24">
      <figure className="w-20">
        <img src="/logo_stylink.jpg" className="h-auto w-full" alt="" />
      </figure>
      <div className="hidden grow justify-around lg:flex">
        {pagesLinks.map((page) => {
          return (
            <Link
              className="text-xl font-bold text-skin-muted transition duration-300 hover:scale-110 hover:text-skin-primary hover:underline"
              to={page.path}
            >
              {page.name}
            </Link>
